...
const UpdateStock = ({ oldData, open, setOpen }) => {
    ...
    const handleOnSubmit = async (data)=>{
        ...
        await dispatch(itemCud({ method: 'put', data: newData, id: oldData.id }))
        fetchData()
        setOpen(false)
        reset()
        setCategory(null)
    ...
    React.useEffect(
        () => {
            ...
            if(oldData && categories){
                ...
                setValue('purchasePrice', oldData.purchasePrice)
                setValue('salePrice', oldData.salePrice)
                setValue('quantity', oldData.stock)
                setValue('commission', oldData.commissionPercent)
            }
            setIsOpen(open)
        }, [open]
    )
