...
const UpdateService = ({ open, setOpen, oldData }) => {
    ...
    const handleOnSubmit = async(data)=>{
        const newData = {...data, shopId: shopId, id: oldData?.id }
        await dispatch(serviceCud({ method: 'put', data: newData, id: oldData?.id }))
        handleClose()
    ...
    React.useEffect(
        () => {
            if(oldData){
                ...
                setValue('normalCommissionPercent', oldData.normalCommissionPercent)
                setValue('bynameCommissionPercent', oldData.bynameCommissionPercent)
            }
            setIsOpen(open)
        }, [open]
    )
